...
// 打开文件
void QHexView::loadFromFile(LPCTSTR aFileName)
{
  // 创建文件内核对象，返回文件内核对象句柄
  mMapFileStruct.hFile = CreateFile(
        aFileName,
        GENERIC_READ,
        FILE_SHARE_READ,
        NULL,
        ...
        NULL);
  if (mMapFileStruct.hFile == INVALID_HANDLE_VALUE) {

    }

  mFileOpened = true;
  if (GetFileSize(mMapFileStruct.hFile, NULL) != 0) {
      // 创建文件映射对象，获得文件映射对象句柄 SEC_IMAGE
      mMapFileStruct.hMapping = CreateFileMapping(
            mMapFileStruct.hFile,
            NULL,
            PAGE_READONLY | SEC_IMAGE,
            0,
            0,
            NULL);
      if (mMapFileStruct.hMapping) {
          // 将文件映射对象映射到进程地址空间，获得映射基址
          mMapFileStruct.ImageBase = MapViewOfFile(
                mMapFileStruct.hMapping,
                FILE_MAP_READ,
                NULL,
                NULL,
                NULL
  ...
  // 判断文件格式
  if (IsPEFile(mMapFileStruct.ImageBase)) {
        mFileOpened = true;
    }

}

bool QHexView::isFileOpened()
{
  return mFileOpened;
}
